...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function pow (uint x, uint n) public pure returns (uint r) {
        r = 1.0;
        while (n > 0) {
            if (n % 2 == 1) {
            r *= x;
    ...
    function reqLoan(uint256 principle, uint rate, uint time) public returns(bool correct) {
        ...
        // A cred uses this function to request the Owner to settle his loan, and the amount to settle is calculated using the inputs.
    ...
    function viewDues(address cred)public view isOwner returns(uint256){
    ...
    function settleDues(address cred) public isOwner returns (bool){
